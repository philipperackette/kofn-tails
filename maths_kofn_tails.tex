...
\subsection{Corps finis premiers}
...
\[
  \F_P = \Z/P\Z
\]
le corps fini à $P$ éléments. Les opérations arithmétiques dans $\F_P$ sont effectuées modulo $P$.
\end{definition}
...
Soit $\mathbb{K}$ un corps commutatif. On note
\[
  \mathbb{K}[X]
\]
l'anneau des polynômes à coefficients dans $\mathbb{K}$.
\end{definition}

\begin{definition}[Espace des polynômes de degré borné]
Soit $\mathbb{K}$ un corps commutatif et $k \in \N$. On pose
\[
  \mathcal{P}_{<k}
  = \{f\in\mathbb{K}[X] \mid \deg(f)<k\}.
\]
\end{definition}

\begin{definition}[Division euclidienne des polynômes]
Soit $\mathbb{K}$ un corps. Pour tous $A(X), B(X) \in \mathbb{K}[X]$ avec $B(X) \neq 0$, il existe un unique couple $(Q(X), R(X))$ tel que :
\[
A(X) = B(X)Q(X) + R(X) \quad \text{avec} \quad \deg(R(X)) < \deg(B(X)) \quad \text{ou} \quad R(X) = 0.
\]
\end{definition}

\subsection{Propriétés algébriques fondamentales}
...
Soit $\mathbb{K}$ un corps et soit $f\in\mathbb{K}[X]$ un polynôme non nul de degré $d\ge 0$. Alors $f$ admet au plus $d$ racines dans $\mathbb{K}$.
\end{lemma}

\begin{proof}
Par récurrence sur $d$. Le cas $d=0$ est immédiat (un polynôme constant non nul n'a aucune racine). Pour $d\ge 1$, si $f$ a une racine $a\in\mathbb{K}$, alors par division euclidienne dans $\mathbb{K}[X]$, on peut écrire $f(X) = (X-a)g(X)$ avec $g\in\mathbb{K}[X]$ de degré $d-1$. En effet, la division de $f$ par $(X-a)$ donne un quotient $g$ et un reste $r$ de degré $<1$, donc constant. En évaluant en $X=a$, on obtient $f(a)=0 = 0\cdot g(a) + r$, donc $r=0$.
...
\subsection{Construction de corps finis par extension}
...
\begin{remark}[Construction de corps finis par quotient]
Soit $\mathbb{F}$ un corps commutatif (par exemple $\F_p$ pour $p$ premier). Soit $m(X) \in \mathbb{F}[X]$ un polynôme irréductible de degré $d$. Alors l'anneau quotient $\mathbb{F}[X]/(m(X))$ est un corps commutatif de cardinal $|\mathbb{F}|^d$. Les éléments de ce corps sont les polynômes de degré au plus $d-1$ à coefficients dans $\mathbb{F}$. Les opérations d'addition et de multiplication sont effectuées modulo $m(X)$. Cette construction est fondamentale pour de nombreux schémas cryptographiques, notamment AES qui utilise $\F_{2^8} = \F_2[X]/(X^8 + X^4 + X^3 + X + 1)$.
\end{remark}

\begin{proposition}[Corps obtenu par quotient avec un polynôme irréductible]
...
Puisque $\mathbb{K}[X]$ est principal (Proposition~\ref{prop:anneau-principal}), et que $m(X)$ est irréductible, l'idéal $I$ est maximal. En effet, soit $J$ un idéal tel que $I \subset J \subset \mathbb{K}[X]$. Alors $J = (p(X))$ pour un certain $p(X) \in \mathbb{K}[X]$. Puisque $m(X) \in J$, on a $p(X) \mid m(X)$. Mais $m(X)$ est irréductible, donc soit $p(X)$ est une constante non nulle (auquel cas $J = \mathbb{K}[X]$), soit $p(X)$ est associé à $m(X)$ (auquel cas $J = I$). Ainsi, $I$ est maximal.

Par la Proposition~\ref{prop:quotient-maximal}, le quotient $\mathbb{K}[X]/I$ est un corps.
\end{proof}

\begin{remark}[Cas des anneaux non principaux]
La démonstration utilise crucialement le fait que $\mathbb{K}[X]$ est un anneau principal. Si on part d'un anneau $A$ quelconque et d'un polynôme $P(X) \in A[X]$ irréductible, l'anneau quotient $A[X]/(P(X))$ n'est généralement pas un corps. Par exemple, $\mathbb{Z}[X]/(X^2 + 1)$ n'est pas un corps (c'est l'anneau des entiers de Gauss $\mathbb{Z}[i]$, où seuls $\pm 1, \pm i$ sont inversibles).
\end{remark}
