...
\author{F2F 60C4 1DAF 4B4A E829 2D4C 7C0C 2DF7 6AE4 D7F5}
\date{\centering\today}
...
\begin{itemize}[leftmargin=2em]
  \item le partage de secret de Shamir sur un corps fini pour répartir un secret maître $S$ selon un seuil $k$-sur-$n$ ;
  \item la dérivation de clés symétriques à partir de $S$ et des mots de passe par HKDF et PBKDF2 ;
  \item une clé de signature Ed25519 dérivée de $S$ (schéma de signature à base de courbe elliptique) ;
  \item un chiffrement hybride RSA-4096 / AES-256-GCM pour les fichiers destinés aux membres du groupe $k$-sur-$n$, la clé RSA privée étant elle-même chiffrée par AES-256-GCM à partir de $S$ ;
\end{itemize}
...
les spécifications usuelles, ce sont les octets $0x36$ (pour \(\mathsf{ipad}\))
...
\begin{definition}[Courbe elliptique Ed25519]
...
L'encodage utilisé pour Ed25519 est l'encodage compressé standard où seul le coordonnée $y$ est stockée avec un bit de parité pour permettre la reconstruction de $x$. L'encodage est en little-endian conformément au standard.
\end{remark}
...
\begin{definition}[Signature Ed25519]
...
\begin{definition}[Vérification Ed25519]
...
\begin{definition}[Fonction de génération de masque MGF1]
...
\begin{definition}[Paramètres RSA-OAEP]
...
\begin{enumerate}[leftmargin=2em]
  \item Calculer $EM = C^d \bmod n$ et convertir en binaire sur $8k$ bits
...
\begin{remark}[Longueur de clé et cohérence opérationnelle]
La norme AES autorise des clés de longueur $128$, $192$ ou $256$ bits.
Dans ce document, et conformément à la procédure opérationnelle retenue,
nous nous restreignons explicitement au cas AES-256, c'est-à-dire $K \in \bin^{256}$.
\end{remark}

\paragraph{Dénombrement des permutations vs. dénombrement des clés.}
...
Pour visualiser l'écart de taille, on passe aux logarithmes en base $2$. En utilisant
l'approximation de Stirling $\log_2(n!) \approx n(\log_2 n - \log_2 e)$,
\[
...
\paragraph{Injectivité pratique.}

Si l'on modélise AES-256 comme une \emph{permutation pseudo-aléatoire} (PRP),
c'est-à-dire comme si, pour chaque clé $K \in \bin^{256}$,
la permutation $E_K$ était tirée uniformément au hasard dans $\mathrm{Sym}(\bin^{128})$,
alors la probabilité que l'application $\Phi : K \mapsto E_K$ ne soit pas injective
est négligeable.

Le risque de collision (existence de $K \neq K'$ tels que $E_K = E_{K'}$)
est donné par le paradoxe des anniversaires dans un ensemble de taille $M = (2^{128})!$ :
\[
  \Pr[\text{collision}] \approx 1 - \exp\left(-\frac{|\mathcal{K}|^2}{2M}\right).
\]
Comme $M$ est extrêmement grand ($\log_2 M \approx 2^{128} \cdot 126{,}56$),
cette probabilité est de l'ordre de
\[
  \frac{|\mathcal{K}|^2}{2M} \approx \frac{2^{512}}{2 \cdot (2^{128})!},
\]
soit $2^{-\Omega(2^{128})}$, totalement négligeable à toute échelle pratique.

En résumé, même si l'injectivité de l'application
\[
  \Phi : \bin^{256} \to \mathrm{Sym}(\bin^{128}),\quad K \mapsto E_K
\]
n'est pas formellement démontrée pour AES-256, le dénombrement ci-dessus justifie
le postulat cryptographique standard : il est computationnellement infaisable
de trouver deux clés distinctes $K \neq K'$ induisant la même permutation de blocs $E_K = E_{K'}$.
...
\begin{definition}[GHASH]
Soit $H\in\F_{2^{128}}$. Pour une séquence $(Y_1,\dots,Y_m)$ de blocs de
$\bin^{128}$, que l'on identifie à $\F_{2^{128}}$, on définit
\[
  \mathrm{GHASH}_H(Y_1,\dots,Y_m)
  = \sum_{j=1}^m H^{m-j+1} \cdot Y_j \in \F_{2^{128}}.
\]
Cette expression correspond à l'itération $((Y_1H + Y_2)H + \dots + Y_m)H$.
\end{definition}

\subsection{Mode AES-GCM}

\begin{definition}[AES-GCM]
Pour une clé $K\in\bin^{256}$, on modélise AES-GCM comme un couple
d'applications
\begin{align*}
  \AESGCM_K &: \bin^{96} \times \bin^* \to \bin^* \times \bin^{128} \\
  \AESGCM_K^{-1} &: \bin^{96} \times \bin^* \times \bin^{128} \to \bin^* \cup\{\perp\}
\end{align*}
où le premier argument est un nonce $N\in\bin^{96}$, le deuxième un message
$M$, la sortie un couple $(C,T)$ avec $C$ le texte chiffré et $T$ le tag (128 bits),
et où la fonction inverse produit soit le message $M$, soit le symbole
d'erreur $\perp$ (hors de $\bin^*$) si la vérification du tag échoue.
\end{definition}

\begin{remark}[Unicité des nonces]
Pour toute clé $K$, les nonces $N$ doivent être uniques. Formellement, si $(N,M) \neq (N',M')$ alors les ensembles de textes chiffrés générés doivent être disjoints avec probabilité écrasante.
\end{remark}

\begin{theorem}[Correction d'AES-GCM]
Pour toute clé $K\in\bin^{256}$, tout nonce $N\in\bin^{96}$ et tout
message $M\in\bin^*$,
\[
  \AESGCM_K^{-1}\bigl(N,\,\AESGCM_K(N,M)\bigr) = M.
\]
\end{theorem}
