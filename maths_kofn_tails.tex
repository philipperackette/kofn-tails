...
\begin{abstract}
On formalise ici, dans un cadre entièrement mathématique, un schéma
$k$-sur-$n$ pour la gestion d'un secret maître, basé sur :
\begin{itemize}[leftmargin=2em]
  \item le partage de secret de Shamir sur un corps fini ;
  \item la dérivation de clés par HKDF et PBKDF2 ;
  \item une signature Ed25519 (courbe elliptique sur un corps fini) ;
  \item un chiffrement hybride RSA--AES avec authentification d'intégrité (AES-GCM).
\end{itemize}
AES-GCM garantit la confidentialité et l'intégrité des données. 
L'authenticité est assurée différemment selon le contexte :
\begin{itemize}[leftmargin=2em]
  \item \textbf{Parts de secret} : authentifiées par les mots de passe individuels (AES-GCM avec clé dérivée du mot de passe)
  \item \textbf{Signatures} : authentifiées cryptographiquement par Ed25519  
  \item \textbf{Fichiers chiffrés hybrides RSA-AES} : intégrité garantie par AES-GCM, mais authenticité de la source non garantie
  \item \textbf{Clés publiques} : doivent être authentifiées après distribution publique par canaux sécurisés
\end{itemize}

Les objets sont définis dans leur structure algébrique naturelle, et
les liens avec les scripts informatiques sont donnés en fin de document.
\end{abstract}
...
Pour chaque clé $K\in\bin^{128}\cup\bin^{192}\cup\bin^{256}$, on
dispose d'une permutation de bloc
\[
  E_K : \bin^{128} \to \bin^{128},
\]
bijective avec réciproque $D_K=E_K^{-1}$.
\end{definition}
  ...
  \item Sels PBKDF2 $S_i$ :
        \begin{itemize}
          \item valeur : bitstrings aléatoires individuels, un par participant,
                générés par \verb|os.urandom(SALT_SIZE)| ;
