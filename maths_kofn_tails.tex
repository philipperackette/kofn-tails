...
\begin{definition}[AES-256 selon FIPS 197]
...
La transformation SUBBYTES() applique une substitution non-linéaire à chaque octet de l'état en utilisant la S-boîte (S-box) d'AES:

Pour chaque octet $b = \{b_7b_6b_5b_4b_3b_2b_1b_0\}$:
\begin{enumerate}
\item Si $b \neq \{00\}$, calculer l'inverse multiplicatif $b^{-1}$ dans GF($2^8$)
...
où $c = \{01100011\}$.
\end{enumerate}
La S-boîte complète est donnée dans la Table 4 du FIPS 197.
\end{definition}
...
La transformation ADDROUNDKEY() combine l'état avec la clé de tour par XOR:
\[
s'_{r,c} = s_{r,c} \oplus w[4 \times round + c]_r \quad\text{pour } 0 \leq r < 4, 0 \leq c < 4
\]
où $w$ est le schedule de clés et $round$ est le numéro du tour.
\end{definition}
...
\begin{enumerate}
\item $\text{state} \leftarrow \text{in}$ (copie de l'entrée dans l'état)
...
\item $\text{out} \leftarrow \text{state}$
\end{enumerate}
\end{definition}
